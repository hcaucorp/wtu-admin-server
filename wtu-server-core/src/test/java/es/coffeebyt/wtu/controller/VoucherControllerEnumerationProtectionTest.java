...
public class VoucherControllerEnumerationProtectionTest {
    ...
    @Before
    public void setUp() {
        testIp = RandomUtils.randomIp();
    }

    @Test
    public void triggerIpBlacklisting_byRedemption() throws Exception {
        Voucher notExistingVoucher = RandomUtils.randomVoucher();
    ...
    private void requestRedemption(Voucher v, int times) throws Exception {
        RedemptionRequest redemptionRequest = new RedemptionRequest(RandomUtils.randomBtcAddress(TestNet3Params.get()), v.getCode());
        ...
        for (int i = 0; i < times; i++)
            mvc.perform(post(url, redemptionRequest)
                    ...
                    .with(VoucherControllerTest.remoteHost(testIp)))
