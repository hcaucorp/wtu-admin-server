...
public class Script {
    ...
    public boolean isSentToCLTVPaymentChannel() {
        ...
        if (!chunks.get(0).equalsOpCode(ScriptOpCodes.OP_IF)) return false;
        // chunk[1] = recipient pubkey
        if (!chunks.get(2).equalsOpCode(ScriptOpCodes.OP_CHECKSIGVERIFY)) return false;
        if (!chunks.get(3).equalsOpCode(ScriptOpCodes.OP_ELSE)) return false;
        // chunk[4] = locktime
        if (!chunks.get(5).equalsOpCode(ScriptOpCodes.OP_CHECKLOCKTIMEVERIFY)) return false;
        if (!chunks.get(6).equalsOpCode(ScriptOpCodes.OP_DROP)) return false;
        if (!chunks.get(7).equalsOpCode(ScriptOpCodes.OP_ENDIF)) return false;
        // chunk[8] = sender pubkey
        return chunks.get(9).equalsOpCode(ScriptOpCodes.OP_CHECKSIG);
    }
