...
public class TransactionSignature extends ECKey.ECDSASignature {
    ...
    public static boolean isEncodingCanonical(byte[] signature) {
        ...
        int hashType = (signature[signature.length - 1] & 0xff) & ~(Transaction.SigHash.ANYONECANPAY.value | Transaction.SigHash.FORKID.value); // mask the byte to prevent sign-extension hurting us
    ...
    public static boolean hasForkId(byte[] signature) {
        int forkId = (signature[signature.length - 1] & 0xff) & Transaction.SigHash.FORKID.value; // mask the byte to prevent sign-extension hurting us
